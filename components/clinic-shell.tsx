...
export function ClinicShell({ className, children }: { className?: string; children: ReactNode }) {
  ...
  return (
    <>
      ...
      <header className="site-header"><div className="shell nav-inner"><Link className="brand" href="/" aria-label="Mission Primary Care home"><Image className="brand-logo" src={LOGO_SRC} alt={LOGO_ALT} width={195} height={90} priority /></Link><nav id="primary-navigation" className={mobileOpen ? 'nav-links open' : 'nav-links'} aria-label="Main navigation">{nav.map(([href, label]) => href === '/services'
        ? <div className="nav-item" key={href}><span className="nav-item-row"><Link href={href} aria-current={pathname === href ? 'page' : undefined} onClick={() => setMobileOpen(false)}>{label}</Link><button type="button" className="nav-toggle" onClick={() => setServicesOpen((v) => !v)} aria-expanded={servicesOpen} aria-label={servicesOpen ? 'Hide services' : 'Show services'}><ChevronDown size={14} /></button></span><div className={servicesOpen ? 'nav-dropdown open' : 'nav-dropdown'}>{services.map((service) => { const Icon = serviceIcons[service.icon]; return <Link href={`/services/${service.slug}`} key={service.slug} onClick={() => setMobileOpen(false)}><span className="nav-dropdown-label"><Icon size={17} />{service.title}</span><ArrowRight size={14} /></Link> })}</div></div>
        : <Link href={href} key={href} aria-current={pathname === href ? 'page' : undefined} onClick={() => setMobileOpen(false)}>{label}</Link>)}<a className="button nav-cta" href={PHONE_TEL} onClick={() => setMobileOpen(false)}><Phone size={16} /> Call {PHONE}</a></nav><div className="nav-actions"><Link className="button button-small" href="/contact">Contact us <ArrowRight size={15} /></Link><button className="menu-button" onClick={() => setMobileOpen(!mobileOpen)} aria-expanded={mobileOpen} aria-controls="primary-navigation" aria-label={mobileOpen ? 'Close menu' : 'Open menu'}>{mobileOpen ? <X /> : <Menu />}</button></div></div></header>
      ...
      <footer className="homepage-footer site-footer">
        ...
        <div className="shell footer-grid">
          <div className="footer-brand-col">
            <Link className="footer-brand" href="/" aria-label="Mission Primary Care home"><Image src={LOGO_SRC} alt={LOGO_ALT} width={184} height={85} /></Link>
