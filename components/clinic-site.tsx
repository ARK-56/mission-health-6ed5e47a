...
export function ClinicSite() {
  ...
  return (
    <main>
      <div className="utility"><div className="shell utility-inner"><span>Care that fits your life.</span><div className="utility-links"><a href="#resources">Patient portal</a><a href="#locations">Find a location</a><a href="tel:18005550140"><Phone size={13} /> 1-800-555-0140</a></div></div></div>
      <header className="site-header"><div className="shell nav-inner"><a className="brand" href="#top" aria-label="Mission Primary Care home"><span className="brand-mark"><Plus size={21} strokeWidth={3} /></span><span>Mission <em>Primary Care</em></span></a><nav className={mobileOpen ? 'nav-links open' : 'nav-links'} aria-label="Main navigation"><a href="#care" onClick={() => setMobileOpen(false)}>Care & services</a><a href="#resources" onClick={() => setMobileOpen(false)}>Patient resources</a><a href="#shop" onClick={() => setMobileOpen(false)}>Wellness shop</a><a href="#about" onClick={() => setMobileOpen(false)}>About us</a></nav><div className="nav-actions"><button className="cart-button" onClick={() => setCartOpen(true)} aria-label={`Open cart, ${cartCount} items`}><ShoppingBag size={19} /> <span className="cart-label">Cart</span>{cartCount > 0 && <b>{cartCount}</b>}</button><button className="button button-small" onClick={openBooking}><CalendarDays size={16} /> Book an appointment</button><button className="menu-button" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Toggle menu">{mobileOpen ? <X /> : <Menu />}</button></div></div></header>
      ...
      <footer id="locations"><div className="shell footer-grid"><div><a className="brand footer-brand" href="#top"><span className="brand-mark"><Plus size={21} strokeWidth={3} /></span><span>Mission <em>Primary Care</em></span></a><p>Care for your whole health, close to home.</p></div><div><strong>Explore</strong><a href="#care">Care & services</a><a href="#shop">Wellness shop</a><a href="#about">About Mission</a></div><div><strong>Get in touch</strong><a href="#locations">Find a location</a><a href="tel:18005550140">1-800-555-0140</a><a href="mailto:hello@missionprimarycare.com">hello@missionprimarycare.com</a></div><div><strong>Stay in the know</strong><p>Healthy insights, delivered monthly.</p><div className="email-input"><input aria-label="Email address" placeholder="Email address" /><button aria-label="Subscribe"><ArrowRight size={17} /></button></div></div></div><div className="shell footer-bottom"><span>© 2026 Mission Primary Care</span><span>Privacy · Accessibility · Terms</span></div></footer>
