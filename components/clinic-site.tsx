...
export function ClinicSite() {
  return (
    <ClinicShell>
      <section className="hero" id="top"><div className="shell hero-grid"><div className="hero-copy hero-stagger"><p className="eyebrow">Primary care, made personal</p><h1>Better health starts with a <i>conversation.</i></h1><p className="hero-text">Your health is personal. Your care should be, too. Mission brings trusted clinicians, thoughtful technology, and more time for what matters to you.</p><div className="hero-actions"><a className="button" href={PHONE_TEL}><Phone size={18} /> Call {PHONE}</a><a className="text-link" href="#care">Explore our care <ArrowRight size={17} /></a></div></div><div className="hero-art" aria-label="The Mission Primary Care clinicians together at the clinic" role="img"><div className="art-window"></div><div className="art-card"><HeartPulse size={19} /><span><strong>Now welcoming new patients</strong><small>Same-day appointments · No referral needed</small><span className="hero-rating"><Stars rating={Math.round(Number(RATING_AVERAGE))} /><small><strong>{RATING_AVERAGE}</strong>/5 from {RATING_TOTAL} reviews</small></span></span></div><div className="art-caption"><span className="caption-line"></span><span>Here for your whole health</span></div></div></div></section>
      <TrustStrip />
      <CareSection />
      <PathwaySection />
      <StorySection />
