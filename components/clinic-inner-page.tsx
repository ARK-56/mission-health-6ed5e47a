...
export function ClinicInnerPage({ kind }: { kind: PageKind }) {
  ...
  const blocks: Record<Block, ReactNode> = {
    ...
    faq: <FaqSection />,
    cta: <CtaSection />,
    content: <section className="section care-section inner-content" data-aos="fade-up"><div className="shell inner-content-grid">{panels.map((section) => <article className="inner-panel" key={section.title}><p className="eyebrow">Mission care</p><h2>{section.title}</h2><p>{section.body}</p>{section.items && <ul>{section.items.map((item) => <li key={item}><Check size={16} />{item}</li>)}</ul>}</article>)}</div></section>,
    highlight: <section className="highlight-band" data-aos="fade-up"><div className="shell highlight-inner"><div><p className="eyebrow">{highlight.label}</p><strong>{highlight.value}</strong><p>{highlight.note}</p></div><div className="highlight-details">{highlight.details.map((detail) => <span key={detail}><Check size={15} />{detail}</span>)}</div></div></section>,
    innerCta: <section className="inner-cta" data-aos="fade-up"><div className="shell"><p className="eyebrow">Now welcoming new patients</p><h2>Let’s make a plan that feels right.</h2><a className="button button-light" href={PHONE_TEL}><Phone size={16} /> Call {PHONE}</a></div></section>,
  }
