...
export function ClinicInnerPage({ kind }: { kind: PageKind }) {
  ...
  const blocks: Record<Block, ReactNode> = {
    ...
    faq: <FaqSection />,
    cta: <CtaSection />,
    content: <section className="section care-section inner-content"><div className="shell inner-content-grid">{panels.map((section, i) => <article className="inner-panel" key={section.title} data-aos="fade-up" data-aos-delay={1000 + (i % 4) * 100}><p className="eyebrow">Mission care</p><h2>{section.title}</h2><p>{section.body}</p>{section.items && <ul>{section.items.map((item) => <li key={item}><Check size={16} />{item}</li>)}</ul>}</article>)}</div></section>,
    highlight: <section className="highlight-band"><div className="shell highlight-inner"><div data-aos="fade-up"><p className="eyebrow">{highlight.label}</p><strong>{highlight.value}</strong><p>{highlight.note}</p></div><div className="highlight-details" data-aos="fade-up" data-aos-delay="1100">{highlight.details.map((detail) => <span key={detail}><Check size={15} />{detail}</span>)}</div></div></section>,
