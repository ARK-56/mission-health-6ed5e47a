...
type Block =
  | 'trust' | 'care' | 'pathway' | 'story' | 'providers' | 'insurance' | 'resources' | 'faq' | 'hours' | 'locations'
...
const layouts: Record<PageKind, Block[]> = {
  ...
  'new-patients': ['pathway', 'content', 'trust', 'care', 'hours', 'resources', 'innerCta'],
...
const pageHighlights: Record<PageKind, { label: string; value: string; note: string; details: string[] }> = {
  services: { label: 'Care that keeps moving', value: 'Mon–Fri', note: 'appointments for everyday and ongoing care', details: ['Preventive visits', 'Chronic condition support', 'Telehealth when you need it'] },
  'new-patients': { label: 'Your first visit', value: 'No referral', note: 'needed to be seen by our team', details: ['Call to book', 'Clear next steps', 'A team that follows up'] },
...
const pageData: Record<PageKind, { eyebrow: string; title: string; intro: string; sections: { title: string; body: string; items?: string[] }[] }> = {
  ...
  insurance: { eyebrow: 'Coverage made clearer', title: 'Care that works with your plan.', intro: 'We are contracted with Medicare Part B, Medi-Cal, and a range of commercial plans and medical groups.', sections: [{ title: 'Plans and groups we work with', body: 'Coverage varies by plan and service. Call our team to confirm your specific plan.', items: ['Medicare Part B', 'Medi-Cal', 'Alameda Alliance', 'Hill Physicians', 'Brown & Toland', 'Nivano', 'Imperial Health', 'Amada', 'UnitedHealthcare PPO', 'Blue Cross (select plans)', 'Alignment Health', 'Scan', 'Aetna', 'Cigna'] }, { title: 'Paying without insurance', body: 'We see self-pay patients. Call the office and our team will walk you through the options before you book.' }] },
...
const additionalSections: Record<PageKind, { title: string; body: string; items?: string[] }[]> = {
  ...
  contact: [{ title: 'Call us', body: `Our team is available Monday through Friday, 9am–5pm at ${PHONE}.` }, { title: 'Visit in person', body: 'Our clinics offer welcoming spaces, primary care, on-site labs, and support in one place.' }, { title: 'Urgent concerns', body: 'For emergencies, call 911. For urgent matters when we are closed, go to the nearest emergency room.' }, { title: 'Send a fax', body: 'Records and referrals can be faxed to 510-796-7797.' }, { title: 'Email us', body: 'For general, non-medical questions, email mpc3755@gmail.com. Please do not send medical information by email.' }],
}

const imageByPage: Record<PageKind, string> = { services: '/clinic-care.png', 'new-patients': '/clinic-care.png', providers: '/clinic-team.png', insurance: '/clinic-care.png', resources: '/clinic-wellness.png', about: '/clinic-team.png', contact: '/clinic-care.png' }
...
export function ClinicInnerPage({ kind }: { kind: PageKind }) {
  ...
  const blocks: Record<Block, ReactNode> = {
    ...
    innerCta: <section className="inner-cta"><div className="shell"><p className="eyebrow">Now welcoming new patients</p><h2>Let’s make a plan that feels right.</h2><a className="button button-light" href={PHONE_TEL}><Phone size={16} /> Call {PHONE}</a></div></section>,
  }
